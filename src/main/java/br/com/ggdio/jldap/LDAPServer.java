...
public class LDAPServer {
    ...
    private static final String LDIF_FILE = "ldif/default.ldif";

    private final DirectoryService directoryService;
    private final org.apache.directory.server.ldap.LdapServer ldapServer;

    public static void main(String[] args) throws Exception {
        final Args cliArguments = new Args();
        final ExtCommander jCmd = new ExtCommander(cliArguments, args);
        jCmd.setProgramName("java -jar ldap-server.jar");
        jCmd.setUsageHead(
                "The ldap-server is a simple LDAP server implementation based on ApacheDS. It creates one user partition with root 'dc=ggdio,dc=com,dc=br'.");
        jCmd.setUsageTail("Examples:\n\n" //
                ...
                + " Starts LDAP server on address 127.0.0.1:389 and imports default data (one user entry 'uid=dio,ou=Users,dc=ggdio,dc=com,dc=br'");
     ...
     * @throws Exception
     */
    public LDAPServer(Args cliArguments) throws Exception {
        ...
        final String host;
        if (Args.ADDRESS.equals(cliArguments.getBindAddress())) {
            host = "127.0.0.1";
        } else {
    ...
    private void importLdif(List<String> ldifFiles) throws Exception {
        if (ldifFiles == null || ldifFiles.isEmpty()) {
            ...
            importLdif(new LdifReader(LDAPServer.class.getResourceAsStream("/" + LDIF_FILE)));
        } else {
